...
- Missing `include`/`select` causing over-fetching
- Inefficient filtering and sorting
- Transaction misuse

**Prioritized Fixes:**
1. **Minimal**: Add `select` to limit fields, use `include` for relations
2. **Better**: Use `findMany` with proper `where`, add database indexes
3. **Complete**: Implement cursor-based pagination, use raw queries for complex aggregations
